...
def _draw_details(subwindow, selected):
  ...
  if selected.line_type == LineType.CIRCUIT_HEADER and selected.circuit.status != 'BUILT':
    ...
    traffic_label = _traffic_label(selected)

    if traffic_label and traffic_label != 'unavailable':
      subwindow.addstr(38, 1, 'traffic: %s' % traffic_label, *attr)
...
def _traffic_label(line):
  ...
  if nyx.cache().collector_status('traffic_counters') == 'available':
    return _traffic_available_without_record_label()
...
def _traffic_available_without_record_label():
